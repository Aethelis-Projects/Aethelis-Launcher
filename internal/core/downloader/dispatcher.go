...
func (d *Dispatcher) downloadSingleURL(
	...
	case http.StatusRequestedRangeNotSatisfiable: // 416: Existing part is invalid
		_ = os.Remove(partPath) // errcheck:ok purge corrupt or mismatched part file before fresh download
	...
	if err := d.verifyPartChecksum(partPath, task); err != nil {
		_ = os.Remove(partPath) // errcheck:ok Checksum failed; purge corrupt file
		if bytesToCount > 0 {
...
func atomicReplace(src, dst string) error {
	if err := os.Rename(src, dst); err != nil {
		...
		_ = os.Remove(dst) // errcheck:ok remove existing destination file on Windows before rename
