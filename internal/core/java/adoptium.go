...
func NewAdoptiumClient(baseURL string, client *http.Client) *AdoptiumClient {
	...
	if client == nil {
		client = netutil.NewHTTPClient("0.6.1", 30*time.Second)
	} else {
		client.Transport = netutil.NewTransport("0.6.1", client.Transport)
	}
...
func (c *AdoptiumClient) GetLatestRelease(ctx context.Context, major int) (*AdoptiumAsset, error) {
	...
	if req.Header.Get("User-Agent") == "" {
		req.Header.Set("User-Agent", netutil.FormatUserAgent("0.6.1"))
	}
